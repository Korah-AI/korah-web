...
- Requested direction:
  ...
  - Add the ability to generate (create) Google Workspace items (Docs, Slides, Sheets) using content generated by the Gemini API.
- Current code:
...
- Questions to resolve:
  ...
  - For generation, which Workspace items (Docs, Slides, Sheets) should we support initially?
  - Do we want to support both import and export in the same release, or phase them?
- Likely initial release slice:
  - For generation: start with Google Docs (as the most common for study guides) and use the Gemini API to generate the content, then use the Google Docs API to create the document.
  - For import: expand supported local import formats first and add source metadata and better file parsing.
- Likely files:
  - `app/korah-chat.js`
  - `study/js/study-api.js`
  - `api/gem-proxy.js`
  - `api/generate-study-item.js`
  - new files for Google Workspace API integration
