
# UI Deisgn To Do

# rBUGs.md
* Do all these I guess :)

# sat/index.html
* Theres like an invisible footer like this at the bottom of the main content area (maybe leave it or get rid of it)

# Question Navigator (questions.html) (*******FOR OSCAR TO FIX*******) (********LOCK IN HAVE FUN**********)
* Fix responsive layouts that are lines 2473-2647 because it's not smoothly being responsive and it's like the width of the navigator changes and it's inside padding. Goal for it is to have 6-7 (67 LOL ;) questions pills per row (At desktop there are 7 per row and at smaller window is 6  per row****FIX THIS*****). Futhermore do whatever updates you think looks best.
* fix dark mode options coloring due to some of the options being black colored pngs and not able to be seen.
* Answer Choice buttons arn't visible sometimes in the container.
* Fix top option hover in sat question container, the first option when hovered, its like top border hover gets cut off
* Check "Clear ALL Data" btn function to make sure it functions properly in all settings modals. 
* Also for the top legends like the check mark with "Correct" text, can make text in dark mode be white and in light mode be black or another more "seen" colors. Same with question pills (if you want)

##### UPCOMING WEEK ##########
* Refine all css files syntaxing and remove duplicates. Make sure there are NO CONFLICTING css classes while making sure each html css and design remains the same. (Do the same with html pages and js files)
* UPDATE UI-UX.md with CLAUDE for better CSS and update AGENT prompting.
* Test and plan everything (find bugs and minor css problems to fix) and plan NEW css updates for pages. IMPROVE THEM.
* LOOK TO IMPROVE COLORING (nothing crazy but make coloring more bold to be seen better on certain pages)
* MAKE SURE EVERYTHING IS IN REM, SO IT'S RESPONSIVE, CHECK EVERY PAGE FOR RESPONSIVENESS AND HOW IT LOOKS WITHOUT SIDEBAR
* PLAN PLAN PLAN PLAN