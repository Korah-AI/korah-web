...
| **API route** | `GET /api/college/c` serving the synced Scorecard dataset (`data/colleges.js`) in the final response shape |
| **Page** | `index.html` — two section sliders (math, reading and writing) with saved-score ticks, live-refilling Safety/Match/Reach columns, band bars on cards, expandable details, filters |
...
- ~~**Phase 2:** the `sync-scorecard.js` script, the real College Scorecard dataset.~~ **Built** — `scripts/sync-scorecard.js` pulls the Scorecard API into `api/college/data/colleges.js` and the route imports it. The frontend did not change, which is what the one-line-source-swap requirement was protecting (§8).
...
└── (data + sync script live outside this folder, next to the route and in scripts/ — see below)
```

Route (under `korah-bot/api/`, same level as `api/sat/`):

```
api/college/c.js                    # GET route; imports and serves the synced dataset
api/college/data/colleges.js         # generated by scripts/sync-scorecard.js — do not edit by hand
scripts/sync-scorecard.js           # run by hand when a new Scorecard year lands, not scheduled (§8)
```
...
> **Naming note:** the folder is `sat/college-match/`. It was built as `college match` with a space, which forced percent-encoded URLs; renamed per §16 (Open Questions #1) so the sidebar href and anchor paths stay plain.
...
## 8 · Phase 2 (built) — sync + dataset

- `scripts/sync-scorecard.js` — run by hand, not on a schedule. Pages the College Scorecard API for operating, bachelor's-predominant schools with a published admit rate and at least 500 undergrads, trims every record to the fields in §4, computes `combined` from the section percentiles, computes school-level `tips` with the same two rules as `college-match.js`, stamps `dataYear`, and writes `api/college/data/colleges.js`.
- `api/college/data/colleges.js` — the synced dataset. It is **committed**, not gitignored: `c.js` imports it statically so Vercel traces it into the function bundle, and nothing fetches the Scorecard API at request time.
- Written as a `.js` module rather than `.json` so the bundler picks it up without relying on JSON import attributes.
- No API key is needed at runtime. The sync script reads one from `--key` or `SCORECARD_API_KEY` and falls back to the public `DEMO_KEY`, which is rate limited to roughly 30 requests an hour, so a cold run waits out the quota between pages.
...
3. **Controls row**
   - **Two section sliders** (range inputs, 200–800, step 10) — math and reading and writing — each with a live value badge in its own tone. The combined score is their sum and is shown as a read-only badge; it is never its own input.
   - **Fixed tick mark** per slider at the student's saved section score, on the track the whole time (§10). A profile holding only a combined score is split evenly across the two sliders so both start somewhere real.
...
- **Band bar — the main visual, not raw numbers.** A horizontal track spanning the school's `combined.p25 → combined.p75`. Marker sits at the current combined score (the two sliders added up). When the score is below p25 or above p75 the marker renders **outside the track** (`.is-below` / `.is-above` states) — that absence of a marker *on* the bar is the point. Raw p25/p50/p75 numerals are secondary text, shown on expand.
- expand-on-click (chevron, not a modal) reveals: `math` / `erw` mini band bars whose markers track the two sliders live, and the `tips` list (school-level) followed by the student-level tip(s).
...
Performance: measured, not assumed. Re-classifying ~1,000 schools costs about 2ms, which is fine once per change — but Alpine reads the `groups` getter about ten times per render pass (count, cards and empty state per column, plus the no-data section), and ten passes is ~19ms, over a frame. `groups` and `states` are therefore memoized in the factory closure, keyed on the two slider values plus the three filters; that puts a drag tick back at ~1.2ms. The memo cells are closure-local on purpose: writing a reactive property from inside a getter would re-trigger the effect that just read it.
