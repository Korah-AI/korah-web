---
name: korah-web-feature-roadmap
overview: Add user-centric features to the Korah web app such as chat history, quick AI tools, settings, automatic prompts, and study modes to make the experience feel like a complete study companion.
todos:
  - id: chat-history
    content: Design and implement persistent and multi-session chat history for the Korah chat widget.
    status: completed
  - id: quick-tools-ui
    content: Add quick AI-powered study tool actions (flashcards, guides, quizzes) integrated directly into the chat UI.
    status: completed
  - id: settings-panel
    content: Create a settings panel for study preferences, accessibility options, and privacy controls.
    status: completed
  - id: auto-prompts
    content: Implement automatic follow-up suggestions and next-step prompts after AI responses.
    status: completed
  - id: mode-toggles
    content: Add study modes/personas (Tutor, Planner, Coach, Exam Cram) that alter Korah’s behavior and UI affordances.
    status: completed
  - id: study-day-automation
    content: Introduce daily start flow, focus session logging, and gentle in-app reminders to structure study days.
    status: in_progress
isProject: false
---
